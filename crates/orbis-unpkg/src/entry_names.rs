...
//! Mapping from a PKG entry id to its conventional `sce_sys` file name.
//!
//! The table is generated verbatim from the original shadPS4 `pkg_type.cpp`
//! and is sorted by id so it can be looked up with a binary search.

/// Well-known PKG entry ids referenced directly by the extractor.
pub mod ids {
    /// `entry_keys`: holds the RSA-wrapped material used to derive DK3.
    pub const ENTRY_KEYS: u32 = 0x10;
    /// `image_key`: the AES-wrapped image key used to derive EKPFS.
    pub const IMAGE_KEY: u32 = 0x20;
    /// `param.sfo`: the PKG metadata blob.
    pub const PARAM_SFO: u32 = 0x1000;
    /// Inclusive range of NP entries that are re-encrypted in place.
    pub const NP_RANGE: std::ops::RangeInclusive<u32> = 0x400..=0x403;
}

/// Entry-id to file-name table, sorted ascending by id.
static ENTRIES: &[(u32, &str)] = &[
    (0x0001, "digests"),
    (0x0010, "entry_keys"),
    (0x0020, "image_key"),
...
/// Returns the conventional file name for a PKG entry id, or an empty string
/// when the id is unknown (matching the original `GetEntryNameByType`).
#[must_use]
...
mod tests {
    ...
    fn known_entries_resolve() {
        assert_eq!(name_for(0x1000), "param.sfo");
        assert_eq!(name_for(0x10), "entry_keys");
        assert_eq!(name_for(0x1200), "icon0.png");
        assert_eq!(name_for(0xDEAD), "");
    }
}
