...
//! RSA-2048 key material used to derive PKG content keys.
//!
//! These are the well-known "fake" (npdrm) keyset and the PKG-derived-key-3
//! keyset transcribed verbatim from the original shadPS4 `keys.h`. Only the
//! components required for private-key operations (`n`, `e`, `d`, `p`, `q`)
//! are kept.

/// A raw RSA private key expressed as big-endian component byte slices.
...
/// The "fake" (npdrm) keyset, used to recover the EKPFS key.
pub static FAKE: RsaComponents = RsaComponents {
    public_exponent: &[0x00, 0x01, 0x00, 0x01],
    modulus: &[
...
/// The PKG-derived-key-3 keyset, used to recover DK3.
pub static PKG_DERIVED_KEY3: RsaComponents = RsaComponents {
    public_exponent: &[0x00, 0x01, 0x00, 0x01],
    modulus: &[
